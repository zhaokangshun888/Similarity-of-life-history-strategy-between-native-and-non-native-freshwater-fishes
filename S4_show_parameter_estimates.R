...
if(file.exists(filename)){
  ...
  for(j in 1:nm){
    ...
    if(m$nr+length(covariates)>1 & m$ns>1){
      ...
      vals = VP$vals
      mycols = colorspace::qualitative_hcl(n = nrow(VP$vals),palette = "Dark 3")   #mycols = rainbow(nrow(VP$vals))
      ...
      plotVariancePartitioning(hM=m, VP=VP, main = paste0("Proportion of explained variance, ",modelnames[j]), cex.main=1.2, cols = mycols, args.leg=list(bg="white",cex=0.8))
      ...
      if(!is.null(R2)){
        ...
        plotVariancePartitioning(hM=m, VP=VPr,main=paste0("Proportion of raw variance, ",modelnames[j]),cex.main=1.2, cols = mycols, args.leg=list(bg="white",cex=0.8),ylim=c(0,1))
      }
    }
  }
  for(j in 1:nm){
    ...
    if(m$nc>1){
      ...
      title(main=mymain, line=2.5, cex.main=1.0)
    }
  }
  for(j in 1:nm){
    m = models[[j]]      
    if(m$nt>1 & m$nc>1){
      ...
      plotGamma(m, post=postGamma, supportLevel = support.level.gamma, param="Sign",
                ...
                cex=c(0.9,0.9,1.0))
      title(main=paste0("GammaPlot ",modelnames[j]), line=2.5,cex.main=1.0)
    }
  }
  for(j in 1:nm){
    m = models[[j]]
    if(m$nr>0 & m$ns>1){
      ...
      for (r in 1:m$nr){
        ...
        corrplot(toPlot[plotOrder,plotOrder], method = "color",
                 col=colorRampPalette(c("#2166ac", "#f7f7f7", "#b2182b"))(3),
                 mar=c(0,0,1,0),
                 main=mymain,cex.main=1.2,tl.cex= 1.0,cl.cex= 1.0)
